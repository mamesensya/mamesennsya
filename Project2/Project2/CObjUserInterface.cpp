...
void CObjUserInterface::Draw() {
	...
	swprintf_s(str, L"ƒXƒe[ƒW%d", playstage);
	...
	if (getflugs) {
		if (waitcount <= 200) {
			if (state){
			swprintf_s(uistr[0], L"@’Êí’e+%d", getguns[0]);
			swprintf_s(uistr[1], L"@ŠÑ’Ê’e+%d", getguns[1]);
			swprintf_s(uistr[2], L"‚R•ûŒü’e+%d", getguns[2]);
			for (int i = 0; i < 3; i++) {
				src.m_top = 0.0f;
				src.m_left = 0.0f;
				src.m_right = src.m_left + 32.0f;
